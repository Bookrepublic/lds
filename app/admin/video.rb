ActiveAdmin.register Video do
  ...
  form do |f|
    ...
    f.inputs "Risorse collegate" do
      f.input :pubblications, as: :check_boxes, label: 'Libri consigliati', collection: Pubblication.order("title ASC").all
      f.input :writers, as: :check_boxes, label: 'Autori consigliati', collection: Writer.order("last_name ASC").all
      #f.has_many :pubblications, allow_destroy: true, new_record: true do |p|
        #p.input :title
    end
    f.inputs "Categorie" do
      f.input :tags, as: :check_boxes, label: 'Categorie consigliate', collection: Tag.order("tag ASC").all
